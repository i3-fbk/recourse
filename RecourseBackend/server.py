...
# Personas for the experiments
personas_files = [
    f for f in os.listdir("data") if os.path.isfile(os.path.join("data", f))
]

# Regenerate personas correctly
personas = dict()
for f in personas_files:
    data_f = json.load(open(os.path.join("data", f), "r"))
    new_format = { k: {elem.get("name"): elem.get("value") for elem in data_f.get(k)} for k in data_f }
    personas[f.split(".")[0]] = new_format
...
def get_user():
    ...
    # We get potential parameters
    persona_id = request.args.get("persona_id", None)
    
    # Get a random user just in case
    x = dataset.sample()
    x = {k: v.to_dict("records")[0] for k,v in x.items()}

    # check whether using the random user or the persona_id
    persona_data = personas.get("persona_"+persona_id, None) if persona_id else x

    if persona_data is None:
        print(f"The requested persona_id is not available: {persona_id}")

    return x if persona_data is None else persona_data

# @app.route("/get_recourse", methods=['GET', 'POST'])
@app.route("/get_recourse_v2", methods=['POST'])
@cross_origin(supports_credentials=True)
def get_recourse():
...
@app.route("/get_recourse_v1", methods=['GET', 'POST'])
@cross_origin(supports_credentials=True)
