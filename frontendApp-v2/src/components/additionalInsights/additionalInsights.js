...
function AdditionalInsight(props) {
  ...
  const marksGeneratorForScaler = (OriginalValue, systemSuggest, index) => {
    const marks = [
      {
        value: OriginalValue,
        label: OriginalValue,
            ...
            featureList.map((item, index) => (
              <div className="AiBox" key={index}>
                ...
                <p className="bodyTextSecondQuestion">
                  2) The maximum affordable increment:{" "}
                </p>
                ...
                <div className="input-container">
                  <Box sx={{ width: 300 }}>
                    <Slider
                      ...
                      defaultValue={item.valueAfter}
                      // getAriaValueText={valuetext}
                      step={1}
                      min={item.min}
                      max={item.max}
                      // marks
                      marks={marksGeneratorForScaler(
                        item.valueBefore,
                        item.valueAfter,
                        index
                      )}
