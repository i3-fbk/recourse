...
function Welcome() {
  ...
  const set_local = async () => {
    ...
    if (old === null) {
      localStorage.setItem("status", JSON.stringify([overalSatisfication]));
    } else {
  ...
  const handleClick = (divId, index, plansDetails) => {
    ...
    logEvent(userID, `smily_interaction#${divId}`, logDetails).then((res) => {
  ...
  function keepThePlan() {
    ...
    logEvent(userID, "keep_plan_button", logDetails).then((res) => {
      console.log("log request done!");
    });
  ...
  const handleOpenModal = () => {
    setIsModalOpen(true);
    logEvent(userID, "Quit_recourse_page_btn", "clicked").then((res) => {
      console.log("log request done!");
    });
  };
  ...
  const handleConfirm = () => {
    ...
    navigate("/quit");
    logEvent(userID, "Quit_recourse_page_YES_btn", "clicked").then((res) => {
      console.log("log request done!");
    });
  };
  ...
  const handleCancel = () => {
    ...
    setIsModalOpen(false);
    logEvent(userID, "Quit_recourse_page_NO_btn", "clicked").then((res) => {
      console.log("log request done!");
    });
  };
  ...
  function resetValues() {
    ...
    setSelectedOptions([]);
    setPreferences({});
    logEvent(userID, "reset_changes", "clicked").then((res) => {
      console.log("log request done!");
    });
  }
    ...
    <Grid>
      ...
      <div className="topMessage">
        <Grid item className="userName">
          ...
          In this page, you can see the available plan. You can choose to keep it if you believe it aligns with the persona's requirements or propose another plan. Additionally, you can modify the plan to make improvements. If you find that none of the suggested plans are satisfactory and your attempts to modify them have not led to an acceptable solution, you can quit the recourse page.
          </div>
        </Grid>
      </div>
      {plans.length === 0 && (
              ...
              {outerItem?.features.map((item, index) => (
                <div key={index} className="PlanDataV1">
                  <div className="titleContainer">
                    ...
                    <Tooltip
                      title={dataset[item.name]?.description}
                      placement="top"
                    >
                  ...
                  <div className="innerDisplayNewPlan">
                    <span className="dataAmountV1">{dataset[item.name].valuesWithLabel ? dataset[item.name].valuesWithLabel[item.valueBefore] : item.valueBefore}</span>
                    <span style={{ padding: "0px 10px" }}>
                      <ArrowRightAltIcon />
                    </span>
                    <span className="dataAmountV1">{dataset[item.name].valuesWithLabel ? dataset[item.name].valuesWithLabel[item.valueAfter] : item.valueAfter}</span>
                  </div>
                </div>
              ))}
            </Grid>
            ...
            {value && (
              <div className="iconsContainer">
                ...
                <Tooltip title="Reset all changes">
                  <RestartAltIcon
                    onClick={resetValues}
                    style={{ color: "#106FDF", cursor: "pointer" }}
                  />
                </Tooltip>
              </div>
            )}
